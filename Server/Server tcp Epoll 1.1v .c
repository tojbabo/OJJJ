	...
	while(1){	//////////////////////////////////////////////////////// 서버 반복부
		...
		{	//////////////////////////////////////////////////////////// 이벤트 확인부
			if(ep_events[i].data.fd==s_sock)											////// 이벤트가 발생한 소켓이 서버소켓일 경우
			{
				...
				/////////////////////////////////////////////////////////// 영상 수신 쓰레드 생성부
				if (pthread_create(&thread_recv, NULL, recv_pic, (void*)&c_adr) != 0) {
...
void* recv_pic(void* arg) {
	struct sockaddr_in target_adr;
	struct sockaddr_in c_adr;
	socklen_t c_adr_sz;
	int sock_pic_recv;
	char msg[BUF_SIZE];

	target_adr = *((struct sockaddr_in*)arg);
	puts("thread create ok");
	...
	printf("sock id is %d\n", sock_pic_recv);
		sock_pic_recv = accept(s_sock, (struct sockaddr*)&c_adr, &c_adr_sz);
		if (target_adr.sin_addr.s_addr == c_adr.sin_addr.s_addr)
			puts("goooood");
}
