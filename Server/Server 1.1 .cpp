...
void* send_face(void* arg) {
	...
	while (1) {
		...
		imencode(".jpg", mat, encoded, compression_params);
		total_pack = 1 + (encoded.size() - 1) / PACK_SIZE;
		sprintf(msg, "%d", total_pack);
		sendto(sock, msg, sizeof(int), 0, (struct sockaddr*)&t_adr, sizeof(t_adr));
		for (int i = 0; i < total_pack; i++)
			sendto(sock, (char*)&encoded[i*PACK_SIZE], PACK_SIZE, 0, (struct sockaddr*)&t_adr, sizeof(t_adr));
	}
}
